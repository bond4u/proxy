#!/usr/bin/env python3
#
# proxy MYLISTADD data cache
#
import time

def log(s):
  print(time.ctime()+" MyListCache: "+s)

class MyListCache():
  '''MYLISTADD data caching'''
  ...
  def parse(self,line):
    ...
    while 0 < len(line):
      ...
      if (-1 != i) and (-1 != j) and (-1 == k or i < k):
        ...
        #log("parse: pair: "+pair)
        line = line[i+1:]
        if ("s=" == pair[0:2]) or ("viewed=" == pair[0:7]) or ("state=" == pair[0:6]):
        ...
        if ("s=" != line[0:2]) and ("viewed=" == line[0:7]) and ("state=" == line[0:6]):
          if 0 < len(key):
            key += "&"
          key += line
  ...
  def load(self):
    '''Loads MYLISTADD info from file'''
    try:
  ...
  def set(self,key,data,append=False):
    '''Add MYLISTADD request and response to cache'''
    wasApp = append
    (req,resp,idate,udate) = self.parse(key)
    if req in self.data:
    ...
    if wasApp or append:
      if append:
        ...
        outf.close()
        if None == self.lastSave:
          self.lastSave = time.time()
      elif wasApp:
        if None == self.lastSave:
          self.lastSave = time.time()
        elif self.lastSave+5*60 < time.time():
          self.save()
          self.lastSave = time.time()
  def getDates(self,key):
    '''Returns MYLISTADD idate and udate'''
    idate = None
    udate = None
    (req,resp,idate,udate) = self.parse(key)
    if req in self.data:
