#!/usr/bin/env python3
#
# proxy MYLISTSTATS data cache
#
import time

def log(s):
  print(time.ctime()+" MyLSCache: "+s)

class MyListStatsCache():
  '''MYLISTSTATS data caching'''
  ...
  def load(self):
    '''Loads MYLISTSTATS info from file'''
    try:
      ...
      for line in inf:
        ...
        if None == self.data:
          if None == idate:
            idate = time.time()
          self.data = { "DATA": resp, "IDATE": idate, "UDATE": udate }
        else:
          self.data["DATA"] += resp
      inf.close()
  ...
  def save(self):
    '''Saves MYLISTSTATS info to file'''
  ...
  def get(self):
    '''Returns cached MYLISTSTATS'''
    r = None
    if None != self.data:
      r = self.data["DATA"]
    return r
  def set(self,data,append=False):
    '''Add MYLISTSTATS request and response to cache'''
    if None != self.data:
      idate = self.data["IDATE"]
      udate = time.time()
    else:
      idate = time.time()
      udate = None
    self.data = { "DATA": data, "IDATE": idate, "UDATE": udate }
    if append:
      self.save()
  def getDates(self):
    '''Returns MYLISTSTATS idate and udate'''
    idate = None
    udate = None
    if None != self.data:
      idate = self.data["IDATE"]
