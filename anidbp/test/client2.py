...
def log(s):
  print(time.ctime()+" client2: "+s,flush=True)
...
enc = "utf-16"
log("main: enc: '"+enc+"'")
req = "AUTH user=me&pass=me&protover=3&client=ommserver&clientver=2&comp=1&enc="+enc
while True:
  r = send(sock,req,"utf-8")
  #200 JIDFx LOGIN ACCEPTED
  resp = recv(sock,"utf-8")
  if None != resp:
    code = getCode(resp)
    if "200" == code:
    ...
    log("main: no-success auth code: "+code)
  time.sleep(1)
  #else keep trying
